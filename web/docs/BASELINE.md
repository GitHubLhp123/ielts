...
- 首页展示 8 个模块，侧边栏入口与卡片都能进入对应页面。
- 直接访问 `/#/<module-id>` 可打开模块，未知路径回到首页。
- 在模块播放音频或启动计时后切换路由，不再继续播放或推进旧会话。
- 刷新页面后，声明支持持久化的状态能够恢复。
- 导入错误格式文件时有明确提示，当前数据不被部分覆盖。
