...
仓库顶层仅保留：`web/`（新版前端）、`legacy/`（旧版归档）、`README.md`、`.gitignore`、`.claude/`。
...
```
web/                 新版前端（Vue3 + Vite + TS + Element Plus + ECharts）
  scripts/           数据与样式工具：sync-vocab-data / sync-corpus-data / scope-legacy-css
  src/modules.ts     ★ 模块注册表
  src/features/*     8 个模块（data/model/domain/persist/stores/components/styles/tests）
